...
```{r}
int_mod_1 <- abalone_recipe %>% step_interact(terms = ~ type:shucked_weight) %>%
                                step_center(all_nominal_predictors()) %>%
                                step_scale(all_nominal_predictors())
int_mod_2 <- abalone_recipe %>%step_interact(terms = ~ longest_shell:diameter) %>%
                               step_center(all_nominal_predictors()) %>%
                               step_scale(all_nominal_predictors())
int_mod_3 <- abalone_recipe %>%step_interact(terms = ~ shucked_weight:shell_weight) %>%
                               step_center(all_nominal_predictors())%>%
                               step_scale(all_nominal_predictors())
```
